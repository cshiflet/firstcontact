...
private slots:
    ...
    void enqueuesAndDecodesLabelLists() {
        ...
        const auto due = fc::cache::PendingOpsRepository::due();
        bool found = false;
    ...
    void markAttemptIncrementsCounter() {
        const qint64 id = fc::cache::PendingOpsRepository::enqueueModify(
            QStringLiteral("msg-2"), {QStringLiteral("STARRED")}, {});
        fc::cache::PendingOpsRepository::markAttempt(id, QStringLiteral("transient"));
        fc::cache::PendingOpsRepository::markAttempt(id, QStringLiteral("transient"));

        for (const auto& op : fc::cache::PendingOpsRepository::due()) {
            if (op.id == id) {
    ...
    void removeDeletesRow() {
        const qint64 id = fc::cache::PendingOpsRepository::enqueueModify(
            QStringLiteral("msg-3"), {}, {QStringLiteral("INBOX")});
